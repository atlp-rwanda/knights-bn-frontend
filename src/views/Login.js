...
import { fetchUsers,validateInput, loginUsersSuccess } from '../redux/actions/index';
...
class Login extends Component {
  constructor(props) {
    super(props);
    this.state = { data: { email: '', password: '' },error:' ', loggedIn:false };
  }
  render() {
  ...
  login = async (e) => {
    ...
    if(validatedFieldsToArray.length && invalidFields.length === 0) {
      await this.props.postThunk('post','/auth/login',loginUsersSuccess,this.props.user);
       const {isLoggedIn}=this.props.userData;
       isLoggedIn?this.props.history.push("/home") :
         this.setState({
           ...this.state,
